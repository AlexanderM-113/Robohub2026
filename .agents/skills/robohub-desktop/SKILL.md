...
`cd desktop\backend && .venv\Scripts\python -m pip install -r requirements.txt` (if needed)
`.\desktop\scripts\build-python.ps1` — produces `desktop/backend/dist/backend.exe`
`.\desktop\scripts\build-electron.ps1` — produces `desktop/dist/RoboticsHub.exe` and `desktop/dist/win-unpacked/RoboticsHub.exe`
...
Launch `desktop/dist/win-unpacked/RoboticsHub.exe` (or open `desktop/electron/index.html` in a browser while `backend.exe` is running).
Wait for `GET http://127.0.0.1:54114/api/` to return `{"message": "Robotics Team Hub offline API"}`.
Log in as owner (`owner@robohub.local` / `Robotics2026!`).
Click the `Team` tab.
Create or select a member and click `Permissions`.
